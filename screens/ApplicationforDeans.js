...
import { OCR_URLON, BASE_URL } from "../config/api";
...
export default function ApplicationforDeans() {
  ...
  const runCrossValidation = async () => {
    try {
      const res = await fetch(`${OCR_URLON}/validate_cross_fields`);
      const json = await res.json();
  ...
  const validateGradeTermMatch = async () => {
    ...
    try {
      const res = await fetch(
        `${OCR_URLON}/results/grade_for_review.txt?t=${Date.now()}`
      );
      if (!res.ok) {
        throw new Error(
  ...
  const validateTamper = async () => {
    const url = `${OCR_URLON}/validate_grade_tamper`;
  ...
  const validateGradeValues = async () => {
    ...
    try {
      ...
      if (
        ...
        typeof OCR_URLON === "string" &&
        OCR_URLON
      ) {
        try {
          const url = `${OCR_URLON}/results/grade_pdf_ocr.txt`;
          const res = await fetch(
  ...
  const handleUpload = async (docType) => {
    ...
    const endpoint =
      docType === "Certificate of Enrollment"
        ? `${OCR_URLON}/upload_registration_summary_pdf`
        : `${OCR_URLON}/upload_grade_pdf`;
    ...
    try {
      ...
      if (docType === "Certificate of Enrollment") {
        ...
        const previewUrl =
          ...
          (json.saved_image ? `${OCR_URLON}/${json.saved_image}` : null);
        ...
        if (json.saved_preview_public_url) {
          ...
          setGradesPreviewUri(
            `${OCR_URLON}/${json.saved_preview}?t=${Date.now()}`
          );
        }
  ...
  async function generatePDFToAppDir(reviewMeta, reviewRows) {
    try {
      ...
      try {
        ...
        const corUrl = `${OCR_URLON}/results/COR_pdf_image.png?t=${Date.now()}`;
        const cogUrl = `${OCR_URLON}/results/qr_website_screenshot.png?t=${Date.now()}`;
      ...
      {currentStep === 5 && (
        <View style={{ flex: 1, padding: 20 }}>
          <View style={[ui.card]}>
            ...
            />
            <Checkbox.Item
              label="I grant permission for the forms and documents to be recorded and saved for review."
              status={consent2 ? "checked" : "unchecked"}
              onPress={() => setConsent2(!consent2)}
              position="leading"
            />
            <Checkbox.Item
              label="I grant permission for the data generated to be posted if needed."
              status={consent3 ? "checked" : "unchecked"}
